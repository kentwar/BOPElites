...
class UKD_mean(BASEacq):
    ...
    def evaluate(self, x, fstar = None):
        x = x.double()
        #post_mean_prob = self.region_probabilities(x)
    ...
    def adjust_fitness(self, fitness, transformed_prob):
        ...
        standardised.
        '''
        maxy = self.fitGP.train_targets.max().item()
        miny = self.fitGP.train_targets.min().item()
        adjustment = maxy - miny
        n_probs = transformed_prob.shape[0]
        adjusted_fit = torch.ones(n_probs) * torch.tensor(adjustment) + fitness.squeeze()
        reduced = torch.tensor(transformed_prob).squeeze() * adjusted_fit
        adjusted_fitness = reduced - adjustment
    ...
    def region_probabilities( self, x ):
        ...
        if fdims == 1:
            ...
            nprobs = np.product([featdimslist[i][0][0] for i in range(len(featdimslist))], axis = 1)[0]    
