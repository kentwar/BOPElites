...
class BOP_UKD_beta_beta(BASEacq):
    '''
    ...
    def __init__(self, fitGP, DGPs, Domain, QDarchive):        
        ...
        self.name = 'BOP_UKD_beta_beta'
        self.cutoff = self.calculate_cutoff()

    def EI(self, x, fstar, beta = 0, min=False , return_mean = False):
        '''
    ...
    def vec_EI(self, x,  fstar_tensor, beta, min=False , return_mean = False):
        ...
        val = torch.sub(meanvect.T,fstar_tensor)
        u = torch.div(val.T ,sig)
        normal = Normal(torch.zeros_like(u), torch.ones_like(u))
        ucdf = normal.cdf(u)
        updf = torch.exp(normal.log_prob(u))

        ei = (sig * ( updf + u * ucdf)) * (1 + beta *  sig)
        if return_mean:
            return( ei, meanvect )
        else:
            return( ei )
    ...
    def grad_vectorised_evaluate(self, x, beta):
        ...
        value = product_reshaped.sum(1).double()
        return(value)
    ...
    def grad_get_ALL_regions(self, x):
        ...
        neighbouring_regions = torch.vstack(neighbouring_regions)
        return(neighbouring_regions)
    ...
    def evaluate(self, x, beta = 0, fstar = None):
        ...
        if x.shape[0] == 1:
            ...
            if torch.eq(x, self.fitGP.train_inputs[0]).all(dim=1).any():
               return((torch.sum(x)*torch.tensor(0)).unsqueeze(0))
    ...
    def vectorised_evaluate(self, x, beta):
        ...
        #for region in region_probs:
            #below_cutoff = region_probs[region] < cutoff
            #fstar_nan = torch.isnan(self.QDarchive.stdfitness[region])
            #needs_cut2 = torch.logical_and(torch.tensor(below_cutoff), fstar_nan)
            #probiszero = torch.logical_or(torch.tensor(region_probs[region])==0, needs_cut2)
        #    probiszero = torch.tensor(region_probs[region])==0
        #    region_probs[region][probiszero] = 0
        ...
        ## Normalise probaabilities
        # probsum = probs.sum(axis = 0 )
        # probsum[probsum == 0] = 1
        # probs= probs / probsum
    ...
    def evaluate_single(self, x , beta, fstar = None):
        ...
        #else:
        regions = self.grad_get_ALL_regions(x)
        region_probs = self.grad_region_probabilities(x, regions)
        #region_probs = self.region_probabilities(x, central_region)

        # Find fstars for each region        
        #fstar_dict = self.find_all_fstar(x, region_probs)
        fstar_tensor = torch.tensor([self.QDarchive.stdfitness[tuple(region)].item() for region in regions],dtype = torch.double)
        ...
        needs_cut = prob_tens < cutoff
        needs_cut2 = torch.logical_and(needs_cut[0], no_fstar)
        probiszero = torch.logical_or(prob_tens ==0, needs_cut2)

        fstar_tensor = fstar_tensor[~probiszero[0]]
        
        if limited:
            prob_tens = prob_tens[~probiszero] / torch.sum(prob_tens[~probiszero])
             #/ torch.sum(prob_tens[~probiszero])
        else:
            prob_tens = prob_tens[~probiszero]
        # Calculate EI and times by probability
        if fstar_tensor.shape[0] == 0:
            return(torch.tensor(0, dtype = torch.double).unsqueeze(0))
        fitness = self.vec_EI( x, fstar_tensor , beta)

        value = torch.sum(fitness.T * prob_tens)#reshape(len(region_probs),)).unsqueeze(0)
    ...
    def check_for_misprediction(self, x ):
        ...
        # region_probs = self.region_probabilities(x, central_region)
        regions = self.grad_get_ALL_regions(x)
        region_probs = self.grad_region_probabilities(x, regions).squeeze(0)
        region_probs = {tuple(regions[c].numpy()):region_probs[c] for c in range(len(regions))}
        # Find fstars for each region        
        fstar_dict = self.find_all_fstar(x, region_probs)
        ...
        for c,region in enumerate(region_probs):            
            values_dict[region] =  self.EI( x, fstar_dict[region], 0 ) * region_probs[region]
        ...
        bestval = values_dict[best_region].item()
        valpercent = (bestval / total_value).item() * 100

        print(f'Max probability {best_prob} in {best_region}, best value {bestval}, {valpercent}% of total')
        if region_probs[best_region] < 0.5 and valpercent > 50:
    ...
    def region_probabilities( self, x , known_region):
        ...
        cutoff = self.cutoff
        # Check if it has a fstar 
        has_fstar = torch.stack([1- np.isnan(self.QDarchive.fitness[r]) for r in regions])
        for c, i in enumerate(has_fstar):
            if i == 0:
                nprobs[c][nprobs[c] < cutoff] = 0
    ...
    def calculate_cutoff(self):
        ...
        #print(f'd: {d}, t: {t}, n: {n}, mis: {mis}, npf: {npf}')
