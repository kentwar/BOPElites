...
class MeanxProb_Parsec(BASEacq):
    '''
    This is the BOP acquisition function for prediction map creation with
    the parsec Domain
    '''
    ...
    def evaluate(self, x, fstar = None):
        x = x.reshape(-1,self.Domain.xdims).double()
        pred_drag = self.dragGP.posterior(x).mean
        pred_lift = self.liftGP.posterior(x).mean
        pred_fit = [self.Domain.calc_fitness(X , pred_drag[c], pred_lift[c]) for c,X in enumerate(x)]
        #prob_feas = self.nanmodel.predict_proba(p)[:,0]
        #fitness = pred_fit * prob_feas
        fitness = torch.tensor(pred_fit)
        return(fitness)
