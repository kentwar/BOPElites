...
class prediction_archive():
    ...
    '''
    def __init__(self, algorithm, PMoptimizer, acq_fun, sideloaded_points = None, **kwargs):
        ...
        if self.return_pred:
            self.true_pred_archive , self.pred_archive  = self.get_PA(n_children, n_generations, sideloaded_points= sideloaded_points)
        else:
            self.true_pred_archive = self.get_PA(n_children, n_generations, sideloaded_points= sideloaded_points)


    def get_PA(self,  n_children, n_iters, sideloaded_points = None):
        '''
    ...
    def seed_predmap(self, pred_archive, acqfun, sideloaded_points = None):
        ...
        if sideloaded_points is not None:
            for x in sideloaded_points:
                index = tuple(self.QDarchive.nichefinder(x).numpy()[0])
                fit = acqfun.evaluate(x)
                fitnan = torch.isnan(fit)
                x_nan = torch.isnan(x).any()
                if not (x_nan and fitnan) and (acqfun.evaluate(x) > pred_archive.fitness[index] or torch.isnan(pred_archive.fitness[index])):
                    pred_archive.genomes[index] = x
