...
class prediction_archive():
    ...
    '''
    def __init__(self, algorithm, PMoptimizer, acq_fun, **kwargs):
        ...
        self.DGPs = None
        try: 
            self.dragGP = algorithm.dragGP
            self.liftGP = algorithm.liftGP
            self.nanmodel = algorithm.nanmodel
        ...
        if self.return_pred:
            self.true_pred_archive , self.pred_archive  = self.get_PA(n_children, n_generations)
        else:
            self.true_pred_archive = self.get_PA(n_children, n_generations)


    def get_PA(self,  n_children, n_iters):
        '''
        ...
        if self.return_pred:
            mean = self.algorithm.fitness.mean()
            std = self.algorithm.fitness.std()
            pred_archive.fitness = pred_archive.fitness*std + mean
    ...
    def seed_predmap(self, pred_archive, acqfun):
        ...
        for index in index_list:
            ...
            fitnan = torch.isnan(pred_archive.fitness[index])
    ...
    def optimize_predmap(self, pred_archive,  n_children, n_iters, known_features = True):
        '''
        optimizes the prediction archive
        '''
        acqfun = self.init_acqfun()
        ...
        for index in index_list:
            ...
            if fit_not_nan:
                ...
                if true_index == index:
                    ...
                    true_archive.fitness[index] = torch.tensor(fitness(x.numpy().reshape(-1, self.domain.xdims))[0])
                else:
                    true_archive.fitness[index] = torch.tensor(0)
        return(true_archive)
    
    def init_acqfun(self):
