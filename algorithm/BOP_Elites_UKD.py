...
class algorithm():
    ...
    def iterate(self, n: int, iteration = None):
        ...
        misprediction_occured = self.acq_fun_eval.check_for_misprediction(new_obs[0])
        ...
        if misprediction_occured and no_improvement:
            self.mispredict +=1    
            self.QDarchive.mispredicted += np.min([self.mispredict,5])
    ...
    def get_diverse_initial_points(self, n: int):
        ...
        init_x = self.gen_elite_children(init_x) # Adds elite children to the initial points
    ...
    def keep_unique_points(self, points):
        ...
        if self.DGPs != None:
            ...
            for c in range(len(index)-1):
                ...
                region_list = torch.cat([region_list, regions.reshape(-1,region_list.shape[-1])])
    ...
    def upscale(self, resolution):
        '''
        Upscales the new BOP to a higher resolution archive.
        '''
        self.printc(f'Upscaling from {self.domain.feature_resolution} to {resolution}')
