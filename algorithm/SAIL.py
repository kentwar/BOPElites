...
class algorithm():
    ...
    def iterate(self, batchsize: int, iteration = None):
        ...
        new_obs = [[p[0], p[1], torch.tensor(p[2])] for p in new_obs]
    ...
    def run_SAIL(self, batchsize):
        ...
        pred_archive = self.optimize_predmap(pred_archive, self.acq_fun_eval, 2**6, 2**8)
    ...
    def acq(self):
        return(self.acq_fun( self.fitGP, self.DGPs, self.domain, self.QDarchive, self.beta))
