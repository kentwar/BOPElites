...
class algorithm():
    ...
    def calculate_progress(self):
        ...
        self.progressarchive = copy.deepcopy(self.QDarchive)
    ...
    def iterate(self, n: int, iteration = None):
        ...
        F = [self.acq_fun_eval.evaluate_single(x.reshape(-1,4),0) for x in X]
        ...
        converged_by_regions = self.noProgress > np.product(self.domain.feature_resolution)
        converged_by_fitness = self.noFitProgress > np.sqrt(np.product(self.domain.feature_resolution))
        converged = converged_by_regions or converged_by_fitness
        if converged:
            self.noProgress = 0
            self.noFitProgress = 0
            resolutions = [[2,2],[5,5],[10,10],[25,25]]
        ...
        if misprediction_occured and no_improvement:
            self.mispredict += 0.2 * np.product(self.QDarchive.feature_resolution)  
            
            self.QDarchive.mispredicted += self.mispredict
             
            cutoff = self.acq_fun_eval.calculate_cutoff()
            self.printc(f'Misprediction detected, adapting cutoff to {cutoff}', color = 'r')
        else:
            self.mispredict = np.max([0, self.mispredict - 0.2 * np.product(self.QDarchive.feature_resolution)])
    ...
    def evaluate_new_point(self, x):
        ...
        new_obs = [x.double() , new_fit, new_desc]
        return(new_obs)
    ...
    def run(self, n_restarts: int, max_iter: int):
        if self.stable_beta:
            ...
            self.beta_range = np.linspace(self.beta, 0, int((max_iter-self.x.shape[0])*8/10))
    ...
    def get_diverse_initial_points(self, n: int):
        ...
        n_random_points = np.clip(m*(self.domain.xdims**2)*np.product(self.domain.feature_resolution),10000,100000)
        ...
        initpoints , initvals = self.select_initial_points()
        x0dict = self.pick_x0(initpoints, initvals, n)
        selected_points = torch.stack(list(x0dict.values()))
        if selected_points.shape[0] < n:
            random_points = self.domain.get_Xsample(n - selected_points.shape[0])
            selected_points = torch.cat([selected_points, random_points])
    ...
    def keep_unique_points(self, points):
        ...
        if self.DGPs != None:
            ...
            bs = int(mem/((np.product(self.domain.feature_resolution))/6))
    ...
    def select_initial_points(self):
        '''
    ...
    def save_best_points(self, X , F):
        ...
        '''
        X = np.array(X)[np.argsort([f for f in F])][:-1]
    ...
    def setup_logging(self):
        ...
        logging.basicConfig(filename=f'{self.save_path}/log.txt', filemode='w', format='%(message)s')
