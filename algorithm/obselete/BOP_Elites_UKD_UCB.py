...
class algorithm():
    ...
    def iterate(self, n: int, iteration = None):
        ...
        #else:
        #    self.mispredict = np.max([0, self.mispredict - 1])
    ...
    def run(self, n_restarts: int, max_iter: int):
        if self.stable_beta:
            ...
            self.beta_range = np.linspace(self.beta, 0, int((max_iter-self.x.shape[0])))
        ...
        for c,region in enumerate(regions):
            ...
            if region not in self.pointdict.keys():
                ...
                if X[c] not in self.pointdict[region]:
                    self.pointdict[region] = torch.cat((self.pointdict[region].reshape(-1,self.domain.xdims), X[c].reshape(-1,self.domain.xdims)))
