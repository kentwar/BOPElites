...
class algorithm():
    ...
    def iterate(self, batchsize: int, iteration = None):
        ...
        if not iteration == None:
            self.printc(f'Iteration {iteration}', log = True)
        
        # Run optimisation
        X = self.run_SAIL(batchsize)
        
        ## Take the entire set of found points, attempt to evaluate them
        accepted_X = []
        iter = 0
        while len(accepted_X) < batchsize:
            new_obs = self.evaluate_new_points(X[batchsize*iter:batchsize*(iter+1)])
            fitisnan = [np.isnan(f) for f in np.array([d[1] for d in new_obs])]
            descisnan = [np.isnan(d).any() for d in np.array([d[4] for d in new_obs])]
            notnan = (~np.array(fitisnan) * ~np.array(descisnan))
            accepted_X = accepted_X + [n for c , n in enumerate(new_obs) if notnan[c]]
            if batchsize*iter+1 > X.shape[0]:
                break
        new_obs = accepted_X[:batchsize]
        ...
        new_obs = [[p[0], p[1], torch.tensor(p[4])] for p in new_obs]
        ...
        self.printc(f'Num points: {self.x.shape[0]}', log = True)
        self.save_data()

        #last_index = self.QDarchive.nichefinder(self.descriptors[-1])
        

    def run_SAIL(self, batchsize):
        '''
    ...
    def evaluate_new_points(self, x):
        ...
        if x.shape == torch.Size([1, self.domain.xdims]):
            new_obs = [[x[0], new_fit[0], new_fit[1], new_fit[2] ,new_desc]]
        else:
            new_obs = [[x[c], new_fit[c][0], new_fit[c][1] , new_fit[c][2], new_desc[c]] for c in range(len(x))]
        return(new_obs)
    ...
    def update_storage(self, observations):
        ...
        notnan = (~torch.isnan(fit) * ~torch.isnan(desc).all(axis = 1))

        self.update_x(x[notnan])
        self.update_fitness(fit[notnan])
        self.update_drag(drag[notnan])
        self.update_lift(lift[notnan])
        self.update_descriptors(desc[notnan])
    ...
    def createmodels(self, ):
        ...
        train_x = self.x.reshape(-1, self.domain.xdims)
    ...
    def run(self, batchsize: int, max_iter: int):
        ...
        i = 1
        while self.x.shape[0] < max_iter:
            if max_iter - self.x.shape[0] < batchsize:
                batchsize = max_iter - self.x.shape[0]
            self.iterate( batchsize, i)
            i += 1

    def acq(self):
        return(self.acq_fun( self.dragGP, self.liftGP, self.DGPs, self.domain, self.QDarchive, self.beta))
    ...
    def plot_predarchive(self, archive , save = True, save_path = None):
        ...
        '''
        true_value_archive = archive.return_copy()
