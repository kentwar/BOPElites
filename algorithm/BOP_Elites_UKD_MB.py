...
class algorithm():
    ...
    def iterate(self, n: int, iteration = None):
        ...
        # converged = converged_by_regions or converged_by_fitness
            ...
            # return(True)
            # self.noProgress = 0
            # resolutions = [[5]*self.domain.fdims,[10]*self.domain.fdims,[20]*self.domain.fdims]
    ...
    def run(self, n_restarts: int,):
        ...
        converged = False
        while not converged:
            converged = self.iterate(n_restarts, len(self.x))
    ...
    def zoom_reinitialise(self, n = None):
        print('Creating models')
        self.createmodels()
        ...
        points = [[self.x[c], self.fitness[c], self.descriptors[c]] for c in range(self.x.shape[0])]
        self.QDarchive.flush()
        self.QDarchive.zoom_initialise(points, False)
    ...
    def zoom_inherit(self, old_algorithm):
        '''
        inherits data from an old algorithm
        '''

        self.x = copy.deepcopy(old_algorithm.x.detach())
        self.fitness = copy.deepcopy(old_algorithm.fitness.detach())
        self.descriptors = copy.deepcopy(old_algorithm.descriptors.detach())
        self.save_path = old_algorithm.save_path
        self.logger = copy.deepcopy(old_algorithm.logger)
        self.zoom_reinitialise()
    # check if stale
